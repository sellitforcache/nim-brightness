...
The error bounds on the measured data were calculated using the uncertainties from statistics, the length of the neutron flight distance, the live time of the detector, the duty cycle of the chopper, the solid angle, and the total proton charge deposited onto the target during the measurement.  In order to keep the count rate acceptably low during the measurement, a very small current (150 $\mu$A) was used on the target.  In this regime, the beam is not very stable and fast beam trips happened frequently.  The beam monitoring system used has a sampling rate of one sample per 5 seconds, and this slow sampling could was a large source of uncertainty.
